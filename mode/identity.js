    ...
    game: {
      ...
      chooseCharacter() {
        ...
        next.setContent(async (event, trigger, player) => {
          ...
          const opponent = game.players.find(p => p !== game.me);
          opponent.identity = "fan";
          opponent.setIdentity("fan");
          
          // 2. Initialize character definitions
          game.me.init("wu_luxun");
          opponent.init("wutugu");
          ...
          if (window.__wuluxunSelectGroupFixed !== true) {
          ...
          // 4. Load skin assets (Classic or Shenxiu Zhengrong)
          const selectedSkin = lib.config.wuluxun_skin === "classic" ? "classic" : "shenxiu_zhengrong";
          const shenxiuSkin = {
            name: "神秀峥嵘",
            image: "image/skin/shenxiu_zhengrong/character/wu_luxun.png",
            audioBase: "audio/skin/shenxiu_zhengrong",
          };
          
          if (selectedSkin === "shenxiu_zhengrong") {
            if (lib.character?.wu_luxun) {
          ...
          // 5. Apply skin styles to Player
          const applySkin = () => {
            if (selectedSkin === "shenxiu_zhengrong") {
              game.me.classList.add("wuluxun-shenxiu-skin");
              if (game.me.node?.avatar) {
                game.me.node.avatar.style.setProperty("background-image", `url("${lib.assetURL}${shenxiuSkin.image}")`, "important");
                game.me.node.avatar.style.setProperty("background-position", "58% 36%", "important");
                game.me.node.avatar.style.setProperty("background-size", "auto 118%", "important");
              }
            }
          };
          applySkin();
          setTimeout(applySkin, 800);
          
          // 6. Native Background Setup
          const openDb = () => {
            return new Promise((resolve, reject) => {
              const request = indexedDB.open("wuluxun_launcher_assets", 1);
          ...
          const getBlob = async () => {
            ...
            return new Promise((resolve, reject) => {
              const request = db.transaction("assets", "readonly").objectStore("assets").get("background_blob");
              request.onsuccess = () => resolve(request.result);
              request.onerror = () => reject(request.error);
            }).finally(() => db.close());
          };
          ...
          const keepMinimal = () => {
            const nodes = [
              ui.config2, ui.pause, ui.auto, ui.sortCard, ui.deckMonitor,
              ui.wuxie, ui.cardPileButton, ui.commonCardPileButton, ui.brawlinfo
            ];
            for (const node of nodes) {
              if (node) {
          ...
          window.wuluxunAudioTest = () => {
            const res = { skin: selectedSkin };
            for (const sk of ["dcxiongmu", "dczhangcai", "dcruxian"]) {
              res[sk] = game.parseSkillAudio(sk, game.me);
              game.trySkillAudio(sk, game.me, true, true);
            }
            return res;
          };
          window.wuluxunSkinAssetTest = async () => {
            const files = ["dcxiongmu1.mp3", "dcxiongmu2.mp3", "dczhangcai1.mp3", "dczhangcai2.mp3", "dcruxian1.mp3", "dcruxian2.mp3", "wu_luxun_die.mp3"];
            const paths = [shenxiuSkin.image, ...files.map(f => shenxiuSkin.audioBase + "/" + f)];
            const res = {};
            await Promise.all(paths.map(async p => {
              try {
                const response = await fetch(lib.assetURL + p, { method: "HEAD", cache: "no-store" });
                res[p] = response.status;
              } catch (err) {
                res[p] = String(err?.message || err);
              }
            }));
            return res;
          };
