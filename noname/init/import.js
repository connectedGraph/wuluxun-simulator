...
async function importCardPack(name) {
  await importFunction("card", `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1/card/${name}`);
}
async function importCharacterPack(name) {
  const alreadyModernCharacterPack = lib.config.moderned_characters || [];
  const path = !alreadyModernCharacterPack.includes(name) ? `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1/character/${name}/index` : `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1/character/${name}`;
...
async function importExtension(name) {
  ...
  try {
    await importFunction("extension", `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1/extension/${name}/extension`);
  } catch (e) {
...
async function importMode(name) {
  ...
  const path = alreadyModernMode.includes(name) ? `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1/mode/${name}/index` : `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1/mode/${name}`;
