...
async function importCardPack(name) {
  await importFunction("card", `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1.0.7/card/${name}`);
}
async function importCharacterPack(name) {
  const alreadyModernCharacterPack = lib.config.moderned_characters || [];
  const path = !alreadyModernCharacterPack.includes(name) ? `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1.0.7/character/${name}/index` : `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1.0.7/character/${name}`;
...
async function importExtension(name) {
  ...
  try {
    await importFunction("extension", `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1.0.7/extension/${name}/extension`);
  } catch (e) {
...
async function importMode(name) {
  ...
  const path = alreadyModernMode.includes(name) ? `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1.0.7/mode/${name}/index` : `https://cdn.jsdelivr.net/gh/connectedGraph/wuluxun-simulator@v1.0.7/mode/${name}`;
